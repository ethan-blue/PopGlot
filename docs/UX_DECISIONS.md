...
主窗口收敛为专注日常使用的生产力工作台，侧栏精简为两大核心视图：**翻译**（双栏输入与对照）与**资料库**（生词本与历史记录的 Master–Detail）。
全部配置解耦至独立的 **SettingsWindow**（设置窗口），划分为 **通用 / 服务 / 快捷键 / 隐私与数据** 四个专区。设置页采用分组标题 + 扁平设置行 + 发丝分隔线，去除多余嵌套卡片；日常修改即时保存或在窗口内草稿解决，日常流程零系统 MessageBox。

### 5. 服务页采用 Master–Detail 架构，草稿内联守卫
...
编辑区引入内联草稿守卫条（DraftGuardBar）与两步确认组件（ConfirmButton），切换未保存修改时在界面内拦截引导，测试连接使用独立内存草稿，不篡改已存凭据；保存与设为默认解耦。

### 6. 视觉风格：冷靛蓝与统一 Token 体系
