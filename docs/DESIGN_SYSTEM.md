...
PopGlot 采用高对比度、低视觉噪音的暗色与亮色调色盘：中性表面为绝对主体，强调色（深蓝紫 #5562B3/#7C89D9）只出现在主操作、选中态、链接等少数位置；语义状态色（成功/警告/危险）与品牌色分开，互不借用。高对比模式开启时，全部 token 换用系统色（窗口色、窗口文字、高亮、热区），异常状态与交互态一并映射。文本 token 对相邻表面的对比度由测试强制 ≥4.5:1，关键控件边界 ≥3:1（AI-RULES §7.1）。
