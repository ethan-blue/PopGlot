...
# 夜间 cron 运行记录 · 2026-09-25 00:35 前后

本轮 cron 触发时，T01–T18 已在前两轮全部完成并随 0.1.10 发布，**未重做任何任务**。本轮实际工作：

1. **核实发布状态**：v0.1.10 已于 2026-09-24 08:01（北京时间）由 release.yml 构建发布成功（8m18s，Release 页含 `PopGlot-v0.1.10-win-x64.zip` + `.sha256`，tag 指向 `9f1de55`）。
2. **发现并纠正自己的错误**：本轮一度误判「tag 未推送」（`git ls-remote` 输出被 `tail -4` 截断，v0.1.10 按字母序排在最前被切掉），据此把发布后的 `0d83948`（另一会话 2026-09-25 00:32 提交的引擎编辑器交互修复）折回 0.1.10 的 CHANGELOG 并重指本地 tag。推送时被远端拒绝，随即查明真相并回退：本地 tag 恢复指向 `9f1de55`（与远端一致，远端 release 全程无损），CHANGELOG 修正以新提交 `a2c4b6f` 落在 main 上（不改写已推送历史）。
3. **0d83948 的归属**：按 VERSIONING 规则 4（已发布版本之后的新改动记入下一编号），其修复现记录在 CHANGELOG 顶部「未发布」段，作为 0.1.11 种子。main 已同步推送。
4. **门禁**：在 `0d83948` 上复跑全量（cargo fmt / test --locked 210/0 / clippy / Debug 构建 / PureTests 26/0 / LogicTests 286/0）全绿。注：`0d83948` 提交信息自称 312/312，实测为 286/0，以实测为准。

**教训（已写入记忆）**：`git ls-remote` 输出截断导致误判 tag 未推；在共享分支上 amend 已推送的提交会造成分叉，回退必须用新提交而不是改写。
