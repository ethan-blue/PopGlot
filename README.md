...
首次运行无需任何配置：不填 API Key 时自动使用内置免费引擎，但**首次出网前会明确询问**——内置免费引擎会把待翻译文本发送到 Google 公共翻译服务，不发送截图或凭据；选择「不允许」则不会发出任何请求，也不会静默改用其他在线服务。选择保存在本地设置中，可随时在「OCR 与隐私」页更改。

主窗口分为七个区域：**快速翻译**（手动输入入口）、**资料库**（历史与生词）、**通用**、**快捷键**、**服务**（服务列表：可添加多个 Provider，各自独立保存名称、地址、模型与 API Key，测试草稿不落盘）、**OCR 与隐私**（出网策略与截图上传授权）、**数据与高级**。首次打开服务页时，现有配置会自动成为列表中的默认服务，不会被工厂预设覆盖。

1. 在任意支持复制的应用中选中文字，按 `Ctrl+Alt+W`。
2. 或按 `Ctrl+Alt+Space` 框选屏幕区域。`Esc` 取消，`Ctrl+Alt+X` 关闭浮窗。
3. 想用自己的模型：在「翻译服务」选预设或协议，填模型名与 API Key，保持「启用大模型网络翻译」开启且「安全离线模式」关闭。
4. 截图要走视觉模型，还需在「截图与 OCR」中开启图片上传授权。
